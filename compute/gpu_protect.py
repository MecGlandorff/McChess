...
Description:
    Small NVIDIA GPU power-limit helper for local McChess training runs.
...
def query_status(gpu: int, runner: Runner = run_command) -> str:
    return checked_nvidia_smi(
        [
            "-i",
            str(gpu),
            "--query-gpu=index,name,power.limit,power.default_limit,power.draw",
            "--format=csv",
        ],
        runner,
    )
...
def format_change(change: PowerLimitChange, dry_run: bool) -> str:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Protected limit as a percent of default power. Default: 75.",
    )
